let str = document.querySelector(".vid-ids").innerText;
let vidarr = str.split(" ");
let i=0;
...
function putRandomVideo() {
    ...
    vid.pause();
    const randomVid = vidarr[Math.floor(Math.random() * vidarr.length)];
    vid.getElementsByTagName('source')[0].src=`videos/${randomVid}`;
    vid.load();
    vid.play();
    vid.loop = true;
}
